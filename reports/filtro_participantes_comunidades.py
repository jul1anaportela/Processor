"""
Filtro interativo de participantes POR COMUNIDADE.

Igual ao filtro por turmas, mas o último nível é a COMUNIDADE. Pergunta, em cascata:
...
COMO USAR:
  ...
  3. python filtro_comunidades.py
...
def main():
    print("=== FILTRO DE PARTICIPANTES (POR COMUNIDADE) ===\n")
    ...
    faltando = [c for c in (COLUNA_LOCALIDADE, COLUNA_PROJETO, COLUNA_COMUNIDADE) if c not in df.columns]
    if faltando:
    ...
    # 3) COMUNIDADES (dentro do projeto escolhido)
    ...
    escolhidas = escolher_varios(comunidades, "Escolha as comunidades (números separados por vírgula, ou 0 para todas): ")

    if escolhidas is None:
        df_final = df_proj
        rotulo_comunidade = "todas"
    else:
        df_final = df_proj[_norm(df_proj[COLUNA_COMUNIDADE]).isin(escolhidas)]
        rotulo_comunidade = escolhidas[0] if len(escolhidas) == 1 else f"{len(escolhidas)}-comunidades"
        print(f"\n>> Comunidades selecionadas: {', '.join(escolhidas)}")

    # 4) SALVA O RESULTADO
    nome_saida = f"02[filtro-comunidade]_{sanitizar(localidade)}_{sanitizar(projeto)}_{sanitizar(rotulo_comunidade)}.xlsx"
